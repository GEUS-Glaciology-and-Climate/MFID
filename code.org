...
test:
	${container_cmd} ${container_args} hillerup/ice_discharge:grass grass ./G/PERMANENT --exec ./dSEC.sh
...
#+BEGIN_SRC bash :results verbatim :tangle dSEC.sh
g.mapset -c dSEC
...
for ((i=0; i<${#times[@]}-1; i++)); do
  ...
  echo "Band $band → $OUTFILE"

  r.external -o \
    source=NetCDF:${INFILE}:ZZ \
    band=${band} \
    output=${OUTFILE}
